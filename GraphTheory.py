...
class Round:
    ...
    def Print(self, print_edges=False):
        ...
        for team in self.teams:
            # team.Print()
            pass
        if print_edges: # only print 100 bc console limitations
            print("Edges:")
            self.edges.sort(key=lambda e: e.weight)
            for edge in self.edges[:100]: # only print the first 100 edges
    ...
    def Cull(self, numCourts=None, printEdges=False): # Select games with lowest weights; if numCourts is specified, select that many games; otherwise, select as many games as possible without repeating teams
    ...
    def ExportHistoryCSV(self, filename="tournament_history.csv"):
        with open(filename, mode="w", newline="") as file:
            ...
            for game in self.games:
                ...
                game_num += 1
    def ReturnHTMLSchedule(self):
        header=None
        rows=None
        if self.numCourts == None:# or self.numCourts == 1:
            header=["Game", "Team1", "Team2"]
            rows=list()
            game_num = 1
            for game in self.games:
                rows.append([game_num, game.team1.name, game.team2.name])
                game_num += 1
        else:
            header = ["Round"]
            rows=list()
...
def GenerateSchedule(all_teams_list, algo_params, num_rounds_sched, num_courts=None, verbose=False):
    ...
    for i in range(num_rounds_sched):
        ...
        if i+1 ==6: 
            master_round.Print(print_edges=True) #DEBUG print the edges 
            selected_games = master_round.Cull(numCourts=num_courts, printEdges=True) #num_courts is the simultaneous number of games to be played
        else:
